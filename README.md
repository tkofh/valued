...
`valued` is a small library that brings structured parsing and type inference to the W3C CSS Value Definition Syntax. The combinators are named after the operators in the spec, so parsers read close to how the spec writes them.
...
- The **output** is a typed structure (`[LengthValue, LineStyleValue, ColorValue]`, with `null` for the parts `someOf` allows you to omit), so you're not discriminating against a generic AST.
- The **input** is a template literal type, so your editor autocompletes keyword-heavy grammars from inside the `parse()` call.

When working with CSS-shaped inputs, `valued` aims to make typing and parsing them as straightforward as it should be.
...
Combinators correspond directly to operators in the CSS grammar.
...
A sequence of parsers (and optional string literals), in order, separated by spaces.
...
At least one parser must match. Any can be omitted; any order.
